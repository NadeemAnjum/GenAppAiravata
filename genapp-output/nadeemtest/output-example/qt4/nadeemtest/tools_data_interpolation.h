void nadeemtest::module_load_tools_data_interpolation()
{
...
//      plt->setPalette( *palette_plot );
   ...
   {
      QLabel * lbl = new QLabel( "output 5  ", this );
      lbl->setMaximumHeight( 22 );
      lbl->show();
      gl->addWidget( lbl, ppos, 0 );
      panel1_sub_widgets.push_back( lbl );
...
//      cb->setPalette( *palette_cb );
      ...
      QString id = "data_interpolation:output5";
      global_data_types[ id ] = "cb";
      panel1_widget_map[ id ] = cb;
      panel1_outputs.push_back( id );
      save_default_value( id );
   ...
   {
      QLabel * lbl = new QLabel( "output again  ", this );
      lbl->setMaximumHeight( 22 );
      lbl->show();
      gl->addWidget( lbl, ppos, 0 );
      panel1_sub_widgets.push_back( lbl );
...
//      cb->setPalette( *palette_cb );
      ...
      QString id = "data_interpolation:output6";
      global_data_types[ id ] = "cb";
      panel1_widget_map[ id ] = cb;
      panel1_outputs.push_back( id );
      save_default_value( id );
   ...
   {
      QLabel * lbl = new QLabel( "test pdb output  ", this );
      lbl->setMaximumHeight( 22 );
      lbl->show();
      gl->addWidget( lbl, ppos, 0 );
      panel1_sub_widgets.push_back( lbl );
      ...
      gl->addWidget( lbl2, ppos, 1 );
      panel1_sub_widgets.push_back( lbl2 );
      connect( lbl2, SIGNAL( pressed() ), this, SLOT( browse_data_interpolation_outputpdb() ) );

      QString id = "data_interpolation:outputpdb";
      global_data_types[ id ] = "outfile";
      panel1_widget_map[ id ] = lbl2;
      panel1_outputs.push_back( id );
      save_default_value( id );
...
void nadeemtest::module_reset_tools_data_interpolation()
{
   reset_default_values();
}

void nadeemtest::module_submit_tools_data_interpolation()
{
   save_last_values();
   ...
   QString program = "/home/nadeem/nadeemtest/bin/data_interpolation";
   ...
   process_json[ "data_interpolation" ] = "";
   qDebug() << "process started";
   string tprojId = createProject("nadeem", "data_interpolation");
   ...
   string texpId = createExperiment("nadeem", "exp1", projId, "data_interpolation", json);	
...
void nadeemtest::readyReadStandardOutput_tools_data_interpolation()
{
   process_json[ "data_interpolation" ] += QString( process_tools_data_interpolation->readAllStandardOutput() );
}

void nadeemtest::readyReadStandardError_tools_data_interpolation()
{
   qDebug() << process_tools_data_interpolation->readAllStandardError();
}

void nadeemtest::error_tools_data_interpolation( QProcess::ProcessError e )
{
   qDebug() << "process_tools_data_interpolation->error()" << e;
}

void nadeemtest::finished_tools_data_interpolation( int, QProcess::ExitStatus )
{
   qDebug() << "process_tools_data_interpolation->finished()";
